...
void direction_deplacement()
{
  ...
  for(size_t j(0);j<tab_player.size();++j)
  {
  	cout<<endl<<"focus sur le joueur "<<j+1<<endl<< endl;
...
void lancer_balles()
{
  cout<<endl<<endl<<"debut analyse balle par joueur"<<endl;
...
bool inters_obst_rect(Rond proie, Player& predateur, Coord obstacle)
{
	...
	if (fabs(proie.x-predateur.getPos_joueur().x)<PRECISION)
	{
	...
	else
	{
		double pente = pente_chemin(predateur.getCoord(),
													   proie_coord);
		double b1 = calcul_b_angle(predateur.getPos_joueur(), predateur.getCible(),pente);  //b1 au dessus 
		double b2 = 2*calcul_b(predateur.getCoord(),pente) - calcul_b_angle(predateur.getPos_joueur(),
																		  predateur.getCible(),pente);
		if(fabs(b1)<fabs(predateur.getPos_joueur().rayon+calcul_b(predateur.getCoord(),pente)))
		{	
			b1=calcul_b(predateur.getCoord(),pente) + predateur.getPos_joueur().rayon;
		    b2=calcul_b(predateur.getCoord(),pente) - predateur.getPos_joueur().rayon;
		}
		...
		if(inter_cote_carre(p1.x,p2.y,p1.y,pente,b1,b2,nbCell))
		{
			cout<<"intersection cote b"<<endl;
			return true;
		}

		if(inter_cote_carre(p3.x,p4.y,p3.y,pente,b1,b2,nbCell))
		{
		...
		if(inter_haut_carre(p2.y,p3.x,p2.x,pente,b1,b2,nbCell))
		{
			cout<<"intersection cote a"<<endl;
			return true;
		}

		if(inter_haut_carre(p1.y,p3.x,p1.x,pente,b1,b2,nbCell))
		{
...
bool chemin_libre(Player& predateur)
{
	...
	for (size_t i(0); i<tab_obst.size(); ++i) // a mettre en decrementation
	{
		if(carre_dans_zone(predateur.getPos_joueur(),
						   ...
						   nbCell))
		{
			cout<<"analyse de l'obstacle en "<<tab_obst[i].getcarre().ligne<<" "<<tab_obst[i].getcarre().colonne<<endl;
